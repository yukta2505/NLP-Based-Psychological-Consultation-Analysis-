...
def explain_therapy(therapy: str, symptoms: List[str], disorder: str) -> str:
    ...
    base_reason = disorder_reasons.get(
        disorder,
        f"{therapy} is recommended based on the presenting symptoms and disorder profile"
    )
...
def _load_disorder_model():
    ...
    if TORCH_AVAILABLE and os.path.isdir(DISORDER_MODEL_PATH):
        try:
            tok = RobertaTokenizerFast.from_pretrained(DISORDER_MODEL_PATH)
            mdl = RobertaForSequenceClassification.from_pretrained(DISORDER_MODEL_PATH)
            mdl.eval()
            result = (mdl, tok)
    ...
    if model is not None:
        try:
            ...
            if os.path.exists(label_map_path):
                ...
                irrelevant = {
                    "admiration", "amusement", "approval", "gratitude",
...
def classify_disorder_with_confidence(text: str) -> Dict[str, Any]:
    ...
    if model is not None:
        try:
            ...
            ml_result = id2label.get(str(max_idx), DISORDER_LABELS[0])
            ...
            ml_prob_max = float(max(probs))

            if ml_result == rule_result:
                # Both agree — use ML confidence directly
                final = ml_result
                conf = ml_conf
                source = "ml+rule"
            elif ml_prob_max >= 0.85:
                # ML is very confident (≥85%) — trust it even if rule disagrees
                final = ml_result
                conf = ml_conf
                source = "ml-dominant"
            elif ml_prob_max >= 0.55:
                # Middle band (55–84%): ML and rule disagree.
                # Rule-based is highly reliable for clinically distinct disorders
                # (Insomnia, Panic Disorder, Stress) — trust its LABEL but blend confidence.
                # This prevents a moderately-confident wrong ML prediction from overriding
                # a clear rule signal (e.g. ML→Anxiety at 70% vs rule→Insomnia).
                _blend_keywords = {
                    "Depression": ["depress", "hopeless", "worthless", "suicidal", "grief",
                                   "anhedonia", "loss of interest", "loss of motivation"],
                    "Anxiety": ["anxi", "worry", "overthink", "nervous", "restless", "panic",
                                "racing thoughts", "apprehensive", "social withdrawal"],
                    "Insomnia": ["insomnia", "awake", "sleepless", "sleep onset", "lying awake",
                                 "cannot sleep", "sleep hygiene"],
                    "Stress": ["stress", "overwhelm", "burnout", "pressure", "tension",
                               "workload", "deadlines"],
                    "Panic Disorder": ["panic attack", "chest tight", "palpitation",
                                       "fear of dying", "interoceptive"],
                }
                tl = text.lower()
                blend_scores = {
                    d: sum(1 for k in kws if k in tl)
                    for d, kws in _blend_keywords.items()
                }
                blend_total = sum(blend_scores.values()) or 1
                best_rule_score = blend_scores.get(rule_result, 1)
                second_blend = sorted(blend_scores.values(), reverse=True)[1] if len(blend_scores) > 1 else 0
                dominance = (best_rule_score - second_blend) / blend_total
                rule_conf_local = round(min(45 + dominance * 200 + (best_rule_score / blend_total) * 30, 92), 1)
                # Trust rule label; blend confidence 40% ML + 60% rule
                final = rule_result
                conf = round(0.40 * ml_conf + 0.60 * rule_conf_local, 1)
                source = "rule+ml-blend"
            else:
                # ML confidence below 55% — rule-based is more reliable
                final = rule_result
                conf = ml_conf  # Still surface the ML prob so UI isn't artificially inflated
                source = "rule"
    ...
    # ── Rule-only fallback (no ML model loaded) ──────────────────────────────
    # Uses the same weighted keyword map as _rule_disorder for consistency.
    ...
    # Phrase-weighted scoring — same weights as _rule_disorder
    _FALLBACK_PHRASE_WEIGHTS = {
        "panic attack": 2.5, "chest tightness": 2.0, "heart palpitation": 2.0,
        "fear of dying": 2.5, "suicidal": 3.0, "passive suicidal": 3.0,
        "anhedonia": 2.5, "psychomotor": 2.0, "loss of interest": 2.0,
        "loss of motivation": 1.8, "worthless": 2.0, "hopeless": 2.0,
        "insomnia": 2.0, "sleep onset": 2.0, "lying awake": 1.8,
        "burnout": 1.8, "overwhelm": 1.5, "significantly impacting": 1.8,
    }

    tl = text.lower()
    scores = {}
    for d, kws in fallback_keyword_map.items():
        scores[d] = sum(_FALLBACK_PHRASE_WEIGHTS.get(k, 1.0) for k in kws if k in tl)

    total = sum(scores.values()) or 1
    best_score = scores.get(rule_result, 1)
    second_best = sorted(scores.values(), reverse=True)[1] if len(scores) > 1 else 0

    # Confidence based on dominance over runner-up + share of total signal.
    # 0 dominance → ~45%, clear winner → up to ~92%
    dominance = (best_score - second_best) / total
    rule_conf = round(min(45 + dominance * 200 + (best_score / total) * 30, 92), 1)
...
def _rule_disorder(text: str) -> str:
    ...
    # Phrase-weighted scoring — longer / more specific phrases get higher weight
    # so "panic attack" counts more than a generic single word like "panic".
    KEYWORD_WEIGHTS = {
        "panic attack": 2.5, "chest tightness": 2.0, "heart palpitation": 2.0,
        "fear of dying": 2.5, "anticipatory anxiety about future attacks": 3.0,
        "suicidal": 3.0, "passive suicidal": 3.0,
        "anhedonia": 2.5, "psychomotor": 2.0,
        "loss of interest": 2.0, "loss of motivation": 1.8,
        "worthless": 2.0, "hopeless": 2.0, "crying spells": 1.8,
        "early morning awakening": 2.0,
        "insomnia": 2.0, "sleep onset": 2.0, "lying awake": 1.8,
        "difficulty initiating sleep": 2.0, "maintaining sleep": 1.8,
        "burnout": 1.8, "overwhelm": 1.5, "unable to switch off": 1.8,
        "social anxiety": 1.8, "school refusal": 1.8,
        "racing thoughts": 1.8, "cognitive restructuring": 1.5,
    }

    scores = {}
    for disorder, keywords in keyword_map.items():
        score = sum(KEYWORD_WEIGHTS.get(kw, 1.0) for kw in keywords if kw in text)
    ...
    if label == "MEDICATION":
        ...
        if len(val) < 6 or " " in val:
            if not any(med in val_lower for med in KNOWN_MEDICATIONS):
                return None
        common_words = {
            "been", "have", "with", "from", "this", "that",
            "they", "them", "then", "when", "what", "some",
...
def clean_entities(entities: Dict[str, List[str]]) -> Dict[str, List[str]]:
    ...
    for label, items in entities.items():
        ...
        for item in (items or []):
            cleaned_item = clean_entity(item, label)
...
def generate_report(analysis: Dict[str, Any]) -> str:
    ...
    date = analysis.get("consultation_date") or parsed.get(
